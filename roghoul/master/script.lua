local gui = loadstring(game:HttpGetAsync("https://raw.githubusercontent.com/z4gs/scripts/master/testtttt.lua"))():AddWindow("Ro-Ghoul | Fixed & Edited by Max", {
...
local tab1, tab2, tab3, tab4, tab5 = gui:AddTab("Main"), gui:AddTab("Target"), gui:AddTab("Farm Options"), gui:AddTab("Trainer"), gui:AddTab("Misc")
...
tab2:AddLabel("Target")

local drop = tab2:AddDropdown("Select", function(opt)
...
if team == "CCG" then tab3:AddLabel("Quinque Stage") else tab3:AddLabel("Kagune Stage") end

local drop2 = tab3:AddDropdown("[ 1 ]", function(opt)
    array.stage = array.stages[tonumber(opt)]
end)

array.stage = "One"

tab3:AddSwitch("Reputation Farm", function(bool) 
    myData:Set("ReputationFarm", bool)
end):Set(myData:Get("ReputationFarm"))

tab3:AddSwitch("Auto Reputation Cashout", function(bool)
    myData:Set("ReputationCashout", bool)
end):Set(myData:Get("ReputationCashout"))

for i,v in pairs(array.boss) do
    tab3:AddSwitch(i.." Boss Farm ".."(".."lvl "..v.."+)", function(bool)
...
tab3:AddSlider("TP Speed", function(x)
    myData:Set("TeleportSpeed", x)
end, {min = 90, max = 250}):Set(45)

tab3:AddSlider("Distance from NPC", function(x)
    myData:Set("DistanceFromNpc", x * -1)
end, {min = 0, max = 8}):Set(65)

tab3:AddSlider("Distance from Bosses", function(x)
    myData:Set("DistanceFromBoss", x * -1)
end, {min = 0, max = 15}):Set(55)

labels.p = {label = tab4:AddLabel("Current trainer: "..player.PlayerFolder.Trainers[team.."Trainer"].Value)}

local progress = tab4:AddSlider("Progress", nil, {min = 0, max = 100, readonly = true})
...
btn2 = tab4:AddButton("Start", function()
...
labels.time = {label = tab4:AddLabel("")}

tab5:AddSwitch("Auto add kagune/quinque stats", function(bool) array.weapon = bool end)
tab5:AddSwitch("Auto add durability stats", function(bool) array.dura = bool end)
tab5:AddSwitch("Auto kick", function(bool) array.kick = bool end)
tab5:AddLabel("Auto kick whitelist (type 1 name per line)")

local console = tab5:AddConsole({
    ["y"] = 50,
    ["source"] = "Text",
})
...
for i,v in pairs(array.skills) do
    tab5:AddSwitch("Auto use "..i.." skill (on bosses)", function(bool)
